/**
 * Cloudflare Worker 临时邮箱转发系统
 * 功能：注册/登录、唯一别名、3-7-30天续期规则、邮件转发自动续期、修改密码、注销、管理后台、禁止注册关键字
 */

// === 默认配置 ===
// 部署后，首次运行会自动创建 SYS:CONFIG。你可以通过管理后台修改这些配置。
const DEFAULT_CONFIG = {
  open_reg: true, 
  domains: ["xxxxxx.xxx"], // 请在后台改为你的真实域名
  admin_email: "xxx@xxx.xxx",// 有后台管理权限的邮箱，请首先用此邮箱注册一下
  blocked_keywords: ["admin", "root", "support", "system", "abuse", "webmaster", "postmaster", "vip"] // 禁止注册的关键字
};

// === HTML 模板 (前端代码) ===
const HTML_TEMPLATE = (body, title = "临时邮箱") => `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 路由分发
    if (url.pathname === '/' || url.pathname === '/login') return renderLogin(env);
    if (url.pathname === '/register') return renderRegister(env);
    if (url.pathname === '/dashboard') return await renderDashboard(request, env);
    if (url.pathname === '/admin') return await renderAdmin(request, env);
  ...
  async email(message, env, ctx) {
    ...
    // 1. 查找别名 (KV key: ALIAS:user.name@domain.com)
    // 强制转小写匹配
    const targetKey = `ALIAS:${recipient.toLowerCase()}`; 
    const aliasData = await env.TM_KV.get(targetKey, 'json');
    ...
    // 2. 检查过期
    if (Date.now() > aliasData.expiresAt) {
      message.setReject("Address expired");
      return;
    }

    // 3. 检查用户状态
    const user = await env.TM_KV.get(`USER:${aliasData.ownerEmail}`, 'json');
    if (!user || user.status === 'banned') {
      message.setReject("User banned");
      return;
    }

    // 4. 执行转发
    try {
      await message.forward(aliasData.ownerEmail);
      
      // 5. 自动续期 (默认触发，忽略时间检查)
      await renewLogic(aliasData, env, targetKey, true);
...
async function handleApi(path, req, env) {
    if (path === '/api/auth/register') return await handleRegister(req, env);
    if (path === '/api/auth/login') return await handleLogin(req, env);
    if (path === '/api/auth/password') return await handleChangePassword(req, env);
    if (path === '/api/auth/delete') return await handleDeleteAccount(req, env);
    
    if (path === '/api/alias/create') return await handleCreateAlias(req, env);
    if (path === '/api/alias/renew') return await handleRenewAlias(req, env);
    if (path === '/api/admin/action') return await handleAdminAction(req, env);
    
    return jsonResp({ error: "Method not found" }, 404);
}

// 续期核心算法
// isAuto = true (邮件触发) -> 忽略时间限制，直接升级到下一阶段
// isAuto = false (手动触发) -> 严格检查剩余时间
async function renewLogic(data, env, key, isAuto) {
    const now = Date.now();
    const timeLeft = data.expiresAt - now;
    const daysLeft = timeLeft / (1000 * 3600 * 24);
    
    let newDurationDays = 30; // 默认

    if (!isAuto) {
        // 手动续期规则检查
        if (data.renewalCount === 0) {
            // 初始阶段(3天)。必须 < 2 天才能续
            if (daysLeft > 2) throw new Error("有效期剩余超过2天，暂不可续期");
            newDurationDays = 7;
        } else if (data.renewalCount === 1) {
            // 阶段1(7天)。必须 < 2 天才能续
            if (daysLeft > 2) throw new Error("有效期剩余超过2天，暂不可续期");
            newDurationDays = 30;
        } else {
            // 阶段2(30天)。必须 < 7 天才能续 ("提取7天续期")
            if (daysLeft > 7) throw new Error("有效期剩余超过7天，暂不可续期");
            newDurationDays = 30;
        }
    } else {
        // 自动续期：直接根据当前阶段升级时长，不报错
        if (data.renewalCount === 0) newDurationDays = 7;
        else newDurationDays = 30;
    }

    // 更新过期时间 (从 Now 开始计算)
    data.expiresAt = now + (newDurationDays * 86400 * 1000);
    data.renewalCount += 1;
    if (isAuto) {
        data.forwardCount = (data.forwardCount || 0) + 1;
    }
    
    await env.TM_KV.put(key, JSON.stringify(data));
}

async function getConfig(env) {
  const conf = await env.TM_KV.get('SYS:CONFIG', 'json');
  if (!conf) return DEFAULT_CONFIG;
  // 确保新字段存在
  if (!conf.blocked_keywords) conf.blocked_keywords = DEFAULT_CONFIG.blocked_keywords;
...
async function getSession(req, env) {
  ...
  return await env.TM_KV.get(`SESSION:${match[1]}`, 'json');
}
...
async function handleRegister(req, env) {
  ...
  if (!config.open_reg) return jsonResp({ error: "系统已关闭注册" }, 403);
  if (await env.TM_KV.get(`USER:${email}`)) return jsonResp({ error: "该邮箱已注册" }, 400);

  const user = {
    email,
    password, // 生产环境请加盐Hash
    role: email === config.admin_email ? 'ADMIN' : 'USER',
    status: 'active',
    createdAt: Date.now()
  };
  
  await env.TM_KV.put(`USER:${email}`, JSON.stringify(user));
...
async function handleLogin(req, env) {
  const { email, password } = await req.json();
  const user = await env.TM_KV.get(`USER:${email}`, 'json');
  
  // 默认管理员初始化后门 (如果admin账号不存在)
  if (email === "admin@system.com" && password === "admin" && !user) {
      const adminUser = { email, password, role: 'ADMIN', status: 'active', createdAt: Date.now() };
      await env.TM_KV.put(`USER:${email}`, JSON.stringify(adminUser));
      const token = `TK-${crypto.randomUUID()}`;
      await env.TM_KV.put(`SESSION:${token}`, JSON.stringify({ email, role: 'ADMIN' }), { expirationTtl: 86400 });
      return jsonResp({ token, redirect: '/admin' });
  }

  if (!user || user.password !== password) return jsonResp({ error: "账号或密码错误" }, 401);
  if (user.status === 'banned') return jsonResp({ error: "账号已被禁用" }, 403);

  const token = `TK-${crypto.randomUUID()}`;
  await env.TM_KV.put(`SESSION:${token}`, JSON.stringify({ email: user.email, role: user.role }), { expirationTtl: 86400 });
  
  return jsonResp({ token, redirect: user.role === 'ADMIN' ? '/admin' : '/dashboard' });
}

// 修改密码
async function handleChangePassword(req, env) {
    const session = await getSession(req, env);
    if (!session) return jsonResp({ error: "未登录" }, 401);
    const { oldPassword, newPassword } = await req.json();
    
    const user = await env.TM_KV.get(`USER:${session.email}`, 'json');
    if (user.password !== oldPassword) return jsonResp({ error: "旧密码错误" }, 400);
    
    user.password = newPassword;
    await env.TM_KV.put(`USER:${session.email}`, JSON.stringify(user));
    return jsonResp({ success: true });
}
...
async function handleDeleteAccount(req, env) {
    ...
    const user = await env.TM_KV.get(`USER:${session.email}`, 'json');
    if (user.role === 'ADMIN') return jsonResp({ error: "管理员不能注销" }, 400);
    
    // 1. 删除别名
    const userAliasKey = `UALIAS:${session.email}`;
    const aliasAddr = await env.TM_KV.get(userAliasKey);
    if (aliasAddr) {
        await env.TM_KV.delete(`ALIAS:${aliasAddr}`);
        await env.TM_KV.delete(userAliasKey);
    }
    
    // 2. 删除用户
    await env.TM_KV.delete(`USER:${session.email}`);
    
    // 3. 删除 Session (前端也需清除Cookie)
    return jsonResp({ success: true });
}

// 创建别名
async function handleCreateAlias(req, env) {
  const session = await getSession(req, env);
  if (!session) return jsonResp({ error: "未登录" }, 401);

  const { prefix, domain } = await req.json();
  const lowerPrefix = prefix.toLowerCase();
  
  // 1. 检查配置中的禁止关键字
  const config = await getConfig(env);
  const blockedList = config.blocked_keywords || [];
  
  // 检查前缀是否包含任何禁止关键字
  for (const keyword of blockedList) {
      if (lowerPrefix.includes(keyword.toLowerCase())) {
          return jsonResp({ error: `前缀包含禁止使用的关键字: "${keyword}"` }, 400);
      }
  }

  const fullAlias = `${lowerPrefix}@${domain}`;
  
  // 检查占用 (排除自己拥有的)
  const exist = await env.TM_KV.get(`ALIAS:${fullAlias}`, 'json');
  if (exist && exist.ownerEmail !== session.email) return jsonResp({ error: "该别名已被占用" }, 400);

  // 删除旧别名 (单别名限制)
  const userAliasKey = `UALIAS:${session.email}`;
  const oldAliasAddr = await env.TM_KV.get(userAliasKey);
  if (oldAliasAddr) {
    await env.TM_KV.delete(`ALIAS:${oldAliasAddr}`);
  }

  // 创建 - 初始 3 天
  const aliasData = {
    ownerEmail: session.email,
    fullAddress: fullAlias,
    createdAt: Date.now(),
    expiresAt: Date.now() + (3 * 86400 * 1000), 
    renewalCount: 0,
    forwardCount: 0
  };

  await env.TM_KV.put(`ALIAS:${fullAlias}`, JSON.stringify(aliasData));
  await env.TM_KV.put(userAliasKey, fullAlias);

  return jsonResp({ success: true });
}

// 续期
async function handleRenewAlias(req, env) {
  const session = await getSession(req, env);
  if (!session) return jsonResp({ error: "未登录" }, 401);

  const userAliasKey = `UALIAS:${session.email}`;
  const aliasAddr = await env.TM_KV.get(userAliasKey);
  if (!aliasAddr) return jsonResp({ error: "未找到别名" }, 404);

  const key = `ALIAS:${aliasAddr}`;
  const data = await env.TM_KV.get(key, 'json');
  
  try {
      await renewLogic(data, env, key, false); // Manual renew
      return jsonResp({ success: true });
  } catch(e) {
...
// 管理员操作
async function handleAdminAction(req, env) {
    const session = await getSession(req, env);
    if (!session || session.role !== 'ADMIN') return jsonResp({ error: "权限不足" }, 403);
    
    const { action, payload } = await req.json();
    
    if (action === 'list_users') {
        const list = await env.TM_KV.list({ prefix: "USER:" });
        const users = [];
        for (const k of list.keys) {
            const u = await env.TM_KV.get(k.name, 'json');
            const ua = await env.TM_KV.get(`UALIAS:${u.email}`);
            u.currentAlias = ua || "无";
            users.push(u);
        }
    ...
    if (action === 'toggle_status') {
        ...
        const u = await env.TM_KV.get(key, 'json');
        if (u) {
            u.status = u.status === 'active' ? 'banned' : 'active';
            await env.TM_KV.put(key, JSON.stringify(u));
        }
        return jsonResp({success: true});
    }

    if (action === 'delete_user') {
        await env.TM_KV.delete(`USER:${payload.email}`);
        const ua = await env.TM_KV.get(`UALIAS:${payload.email}`);
        if (ua) await env.TM_KV.delete(`ALIAS:${ua}`);
        await env.TM_KV.delete(`UALIAS:${payload.email}`);
        return jsonResp({success: true});
    }

    if (action === 'get_config') return jsonResp(await getConfig(env));
    
    if (action === 'update_config') {
        await env.TM_KV.put('SYS:CONFIG', JSON.stringify(payload));
        return jsonResp({success: true});
    }
...
function renderLogin(env) {
  return new Response(HTML_TEMPLATE(`
    <div class="flex flex-col items-center justify-center min-h-screen p-4" x-data="{ email: '', password: '', isLogin: true }">
    <h1 id="tmd"> </h1>
    <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-md fade-in">
        <h1 class="text-2xl font-bold text-center mb-6 text-brand-600">邮箱转发系统</h1>
        <div class="flex gap-4 mb-6 bg-slate-100 p-1 rounded-lg">
          <button @click="isLogin = true" :class="isLogin ? 'bg-white shadow text-blue-600' : 'text-slate-500'" class="flex-1 py-2 rounded-md transition font-medium">登录</button>
          <button @click="isLogin = false" :class="!isLogin ? 'bg-white shadow text-blue-600' : 'text-slate-500'" class="flex-1 py-2 rounded-md transition font-medium">注册</button>
        </div>
        <form @submit.prevent="submit">
          <div class="space-y-4">
            <input type="email" x-model="email" placeholder="真实邮箱" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 outline-none">
            <input type="password" x-model="password" placeholder="密码" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 outline-none">
            <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition font-bold" x-text="isLogin ? '登录' : '注册'"></button>
          </div>
        </form>
        <div id="msg" class="mt-4 text-center text-sm text-red-500 font-medium"></div>
        <div  class="mt-4 text-center text-sm text-500 font-medium">Powered by <a aria-label="GitHub Homepage" class="footer-octicon mr-2" href="https://github.com/Y560H/ForwardMail">Y560H</a></div>
      </div>
    </div>
    <script>
      function submit() {
        const data = this.$data;
        const url = data.isLogin ? '/api/auth/login' : '/api/auth/register';
        document.getElementById('msg').innerText = '处理中...';
        fetch(url, { method: 'POST', body: JSON.stringify(data) }).then(r => r.json()).then(res => {
            if (res.error) document.getElementById('msg').innerText = res.error;
            else if (data.isLogin) {
                 document.cookie = "token="+res.token+"; path=/; max-age=86400";
                 window.location.href = res.redirect;
            } else { alert('注册成功，请登录'); location.reload(); }
        });
      }
    </script>
...
async function renderDashboard(req, env) {
  const session = await getSession(req, env);
  if (!session) return new Response(null, { status: 302, headers: { Location: '/' } });

  const config = await getConfig(env);
  const userAliasAddr = await env.TM_KV.get(`UALIAS:${session.email}`);
  const aliasData = userAliasAddr ? await env.TM_KV.get(`ALIAS:${userAliasAddr}`, 'json') : null;
  ...
  let stageLabel = "";
  if (aliasData) {
      const diff = aliasData.expiresAt - Date.now();
      if (diff <= 0) { isExpired = true; timeLeftStr = "已过期"; }
      else {
          const d = Math.floor(diff / 86400000);
          const h = Math.floor((diff % 86400000) / 3600000);
          timeLeftStr = `${d}天 ${h}小时`;
      }
      
      if(aliasData.renewalCount === 0) stageLabel = "初始阶段 (3天)";
      else if(aliasData.renewalCount === 1) stageLabel = "第一次续期 (7天)";
      else stageLabel = "稳定阶段 (30天)";
  }

  return new Response(HTML_TEMPLATE(`
    <div class="max-w-4xl mx-auto p-4" x-data="{ tab: 'home', prefix: '${aliasData ? aliasData.fullAddress.split('@')[0] : ''}', domain: '${aliasData ? aliasData.fullAddress.split('@')[1] : config.domains[0]}', oldPwd: '', newPwd: '' }">
      <nav class="flex justify-between items-center py-4 mb-8 border-b">
        <div class="font-bold text-xl text-blue-600">邮箱转发系统</div>
        <div class="flex items-center gap-4">
           <button @click="tab='home'" :class="tab=='home'?'text-blue-600 font-bold':''">首页</button>
           <button @click="tab='settings'" :class="tab=='settings'?'text-blue-600 font-bold':''">设置</button>
           <button onclick="document.cookie='token=; max-age=0';location.href='/'" class="text-red-500">退出</button>
        </div>
      </nav>

      <div x-show="tab === 'home'" class="space-y-6 fade-in">
        <!-- 别名卡片 -->
        <div class="bg-white rounded-xl shadow p-6 border-l-4 ${aliasData && !isExpired ? 'border-green-500' : 'border-slate-300'}">
          <div class="flex justify-between mb-2">
             <h2 class="text-lg font-bold text-slate-800">当前临时邮箱</h2>
             ${aliasData ? `<span class='text-xs bg-gray-100 px-2 py-1 rounded text-gray-600'>${stageLabel}</span>` : ''}
          </div>
          ${aliasData ? `
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
               <div>
                  <div class="text-3xl font-mono font-bold text-blue-600 select-all">${aliasData.fullAddress}</div>
                  <div class="text-sm text-slate-500 mt-1">转发至: ${session.email} | 转发次数: ${aliasData.forwardCount||0}</div>
               </div>
               <div class="text-right">
                  <div class="text-2xl font-bold ${isExpired ? 'text-red-500' : 'text-green-600'}">${timeLeftStr}</div>
                  <div class="text-xs text-slate-400">有效期至: ${new Date(aliasData.expiresAt).toLocaleString()}</div>
               </div>
            </div>
            <div class="mt-6 flex gap-4">
               <button onclick="renew()" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 font-medium">立即续期</button>
               <button onclick="simulateEmail()" class="px-4 py-2 border border-gray-300 text-gray-600 rounded hover:bg-gray-50 text-sm">模拟收信 (测试续期)</button>
            </div>
            <p class="text-xs text-slate-400 mt-2">* 只有在剩余时间 < 2天 (或稳定期 < 7天) 时才可手动续期。收到邮件自动续期。</p>
          ` : `<div class="text-slate-500 py-4">您还没有创建临时邮箱。</div>`}
        </div>

        <!-- 创建/修改 -->
        <div class="bg-white rounded-xl shadow p-6">
          <h3 class="font-bold text-slate-800 mb-4">${aliasData ? '更换邮箱 (注意：有效期将重置为3天)' : '创建新邮箱'}</h3>
          <div class="flex gap-2">
             <input type="text" x-model="prefix" placeholder="前缀" class="flex-1 px-3 py-2 border rounded">
             <span class="self-center">@</span>
             <select x-model="domain" class="px-3 py-2 border rounded bg-white">
                ${config.domains.map(d => `<option value="${d}">${d}</option>`).join('')}
             </select>
             <button onclick="createAlias()" class="bg-blue-600 text-white px-6 py-2 rounded">提交</button>
          </div>
        </div>
      </div>

      <div x-show="tab === 'settings'" class="space-y-6 fade-in" x-cloak>
          <div class="bg-white rounded-xl shadow p-6">
             <h3 class="font-bold mb-4">修改密码</h3>
             <input type="password" x-model="oldPwd" placeholder="旧密码" class="w-full px-3 py-2 border rounded mb-3">
             <input type="password" x-model="newPwd" placeholder="新密码" class="w-full px-3 py-2 border rounded mb-3">
             <button onclick="changePwd()" class="bg-blue-600 text-white px-4 py-2 rounded">保存</button>
          </div>
          <div class="bg-red-50 rounded-xl shadow p-6 border border-red-100">
             <h3 class="font-bold text-red-800 mb-2">注销账号</h3>
             <p class="text-sm text-red-600 mb-4">删除账号是不可逆的操作，您的邮箱别名将立即失效。</p>
             <button onclick="deleteAccount()" class="bg-red-600 text-white px-4 py-2 rounded">确认注销</button>
          </div>
      </div>
    </div>
    <script>
      function createAlias() {
         ...
         fetch('/api/alias/create', { method: 'POST', body: JSON.stringify({ prefix: data.prefix, domain: data.domain }) })
           .then(r=>r.json()).then(res=>{ if(res.error) alert(res.error); else location.reload(); });
      }
      function renew() {
         fetch('/api/alias/renew', { method: 'POST' }).then(r=>r.json()).then(res=>{ if(res.error) alert(res.error); else { alert('续期成功'); location.reload(); } });
      }
      function simulateEmail() {
         // This is a dummy function just to show user can test renewal. In reality, email comes from Worker.email()
         // But we can call renew with force? No, user API is protected.
         alert("请向该邮箱发送真实邮件来测试自动续期，或者等待时间满足条件后手动续期。");
      }
      function changePwd() {
         const data = document.querySelector('[x-data]')._x_dataStack[0];
         fetch('/api/auth/password', { method: 'POST', body: JSON.stringify({ oldPassword: data.oldPwd, newPassword: data.newPwd }) })
           .then(r=>r.json()).then(res=>{ if(res.error) alert(res.error); else { alert('修改成功'); data.oldPwd=''; data.newPwd=''; } });
      }
      function deleteAccount() {
         if(!confirm('真的要注销吗？')) return;
         fetch('/api/auth/delete', { method: 'POST' }).then(r=>r.json()).then(res=>{ 
             if(res.error) alert(res.error); 
             else { alert('注销成功'); document.cookie='token=; max-age=0'; location.href='/'; } 
         });
      }
    </script>
  `, "Dashboard"), { headers: { "Content-Type": "text/html" } });
}

async function renderAdmin(req, env) {
  const session = await getSession(req, env);
  if (!session || session.role !== 'ADMIN') return new Response("Access Denied", { status: 403 });

  return new Response(HTML_TEMPLATE(`
    <div class="max-w-5xl mx-auto p-4" x-data="adminApp()">
      <nav class="flex justify-between items-center py-4 mb-8 border-b">
        <div class="font-bold text-xl text-purple-700">管理后台</div>
        <a href="/dashboard" class="text-sm text-blue-600 hover:underline">返回用户中心</a>
      </nav>

      <div class="bg-white p-6 rounded-xl shadow mb-6">
         <h3 class="font-bold mb-4">系统设置</h3>
         <div class="flex items-center gap-4 mb-4">
            <span>开放注册:</span>
            <button @click="toggleReg" x-text="config.open_reg ? '开启' : '关闭'" :class="config.open_reg?'bg-green-500':'bg-red-500'" class="text-white px-3 py-1 rounded text-sm"></button>
         </div>
         <div class="mb-4">
            <div class="text-sm mb-1">域名后缀 (逗号分隔):</div>
            <input type="text" x-model="domainStr" class="w-full border px-2 py-1 rounded mb-2">
         </div>
         <div class="mb-4">
            <div class="text-sm mb-1">禁止注册关键字 (逗号分隔, 包含即禁止):</div>
            <textarea x-model="blockedStr" class="w-full border px-2 py-1 rounded h-20" placeholder="admin, root..."></textarea>
         </div>
         <button @click="saveSettings" class="bg-purple-600 text-white px-4 py-2 rounded text-sm">保存系统设置</button>
      </div>

      <div class="bg-white rounded-xl shadow overflow-hidden">
         <div class="p-4 border-b flex justify-between">
            <h3 class="font-bold">用户列表</h3>
            <button @click="loadUsers" class="text-sm text-blue-600">刷新</button>
         </div>
         <table class="w-full text-sm text-left">
            <thead class="bg-slate-50 text-slate-500"><tr><th class="p-3">用户</th><th class="p-3">别名</th><th class="p-3">状态</th><th class="p-3 text-right">操作</th></tr></thead>
            <tbody>
               <template x-for="u in users">
                  <tr class="border-t hover:bg-slate-50">
                     <td class="p-3" x-text="u.email"></td>
                     <td class="p-3 text-blue-600" x-text="u.currentAlias"></td>
                     <td class="p-3" x-text="u.status"></td>
                     <td class="p-3 text-right space-x-2">
                        <button @click="toggleStatus(u)" class="text-orange-600 hover:underline">切换状态</button>
                        <button @click="delUser(u)" class="text-red-600 hover:underline">删除</button>
                     </td>
                  </tr>
               </template>
            </tbody>
    ...
    <script>
       function adminApp() {
          return {
             config: { open_reg: true, domains: [], blocked_keywords: [] }, 
             domainStr: '', 
             blockedStr: '',
             users: [],
             init() { this.loadConfig(); this.loadUsers(); },
             api(action, payload) { return fetch('/api/admin/action', { method: 'POST', body: JSON.stringify({ action, payload }) }).then(r=>r.json()); },
             loadConfig() { 
                 this.api('get_config').then(res => { 
                     this.config = res; 
                     this.domainStr = (res.domains || []).join(','); 
                     this.blockedStr = (res.blocked_keywords || []).join(',');
                 }); 
             },
             saveSettings() { 
                 this.config.domains = this.domainStr.split(',').map(s=>s.trim()).filter(s=>s);
                 this.config.blocked_keywords = this.blockedStr.split(',').map(s=>s.trim()).filter(s=>s);
                 this.api('update_config', this.config).then(()=>alert('保存成功')); 
             },
             toggleReg() { this.config.open_reg = !this.config.open_reg; },
             loadUsers() { this.api('list_users').then(res => this.users = res); },
             toggleStatus(u) { this.api('toggle_status', { email: u.email }).then(() => this.loadUsers()); },
             delUser(u) { if(confirm('确定删除?')) this.api('delete_user', { email: u.email }).then(() => this.loadUsers()); }
          }
       }
    </script>
